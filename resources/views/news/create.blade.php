...
@section('content')
<form action="{{ route('admin.news.store') }}"  class="form" method="POST">
    @csrf
    ...
    <div class="form-group">
        ...
        <textarea name="content" value="{{ old('content') }}" class="form-control" placeholder="Content" required></textarea>
    </div>
    <div class="form-group">
        <label for="InputState">State</label>
        <select class="form-control selectpicker", name="status" required>
            @foreach( \Domain\News\Status::displayNameList() as $key => $value)
                <option class="btn btn-link" value="{{ $key }}">{{ $value }}</option>
            @endforeach
        </select>
    </div>
